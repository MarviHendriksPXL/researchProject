  ...
  c-1.407,1.406-2.197,3.313-2.197,5.303c0.001,1.99,0.791,3.896,2.198,5.305L94.861,156.507z" />
    ...
    <div class="login">
      <form @submit.prevent="sendResetLink">
        <div>
          <div>
            <p>We sturen een reset wachtwoord link naar uw emailadres.</p>
          </div>
          <div>
            <input v-model="email" type="email" name="email" class="form-control" placeholder="Geef uw emailadres in" required autofocus />
...
export default {
  ...
  methods: {
    ...
    async sendResetLink () {
      try {
        this.errorMessage = ''
        await this.global.findAllUsers()
        for (const user of this.global.allUsers) {
        ...
        await this.global.confirmationPopUp(`${this.email} bestaat niet`, 'Gelieve een correct email adres in te geven', 'Ok')
        this.email = ''
