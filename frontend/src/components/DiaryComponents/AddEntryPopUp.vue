<template>
    ...
    <div class="pop-up">
        <h1>Dagboek invulling</h1>
      <form @submit.prevent="addEntry">
        <textarea v-model="entry" required autofocus></textarea>
        <div class="buttons">
            <button type="submit" class="addButton">Toevoegen</button>
...
export default {
  ...
  methods: {
    async addEntry () {
      try {
        await this.global.addEntryToDiary(this.mareId, this.entry)
        this.closePopUp()
      } catch (error) {
